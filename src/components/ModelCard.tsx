...
export function ModelCard({ model, tier, onClick }: ModelCardProps) {
  ...
  return (
    <div
      className={`model-card ${isFree ? 'model-card--free' : ''}`}
      onClick={handleClick}
    ...
    >
      ...
      <div className="card-header">
        <div className="card-header-flex">
          <div className="card-header-row">
            <div className="model-name" title={sanitizeModelName(model.name)}>{sanitizeModelName(model.name)}</div>
            <CopyButton text={model.name} label="name" />
          </div>
          <div className="card-header-row card-header-row-mt">
            <div className="model-id" title={sanitizeModelId(model.model_id)}>{sanitizeModelId(model.model_id)}</div>
      ...
      <div className="card-stats">
        <div className="stat-chip">
          ...
          {formatCtx(model.context_window ?? 0)} ctx
        </div>
        {releaseYear && (
          <div className="stat-chip">
